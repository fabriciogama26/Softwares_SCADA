...
class ServidorMODBUS():
    ...
    def run(self):
        ...
        try:
            ...
            while True:
                # set_holding_registers ou set_words
                # codigo para teste (''random.randrange(int(0.95*400),int(1.05*400))'') 
                # Registrador na posição 1000
                self._db.set_holding_registers(1000,[random.randrange(int(0.95*400),int(1.05*400))])
                print('======================')
                print("Tabela MODBUS")
                # Leitura do registrador na posição 1000 e no 2000 é para ver no cliente
